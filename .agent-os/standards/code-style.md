...
- Use single quotes only when interpolation is needed
...
<conditional-block task-condition="jsx" context-check="jsx-style">
IF current task involves writing or updating JSX:
  ...
  ELSE:
    <context_fetcher_strategy>
      IF current agent is Claude Code AND context-fetcher agent exists:
        USE: @agent:context-fetcher
        REQUEST: "Get JSX style rules from code-style/JSX-style.md"
        PROCESS: Returned style rules
      ELSE:
